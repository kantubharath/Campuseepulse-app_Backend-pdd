...
router.get("/tentative", async (req, res) => {
    try {
        const snapshot = await db.collection('tentativeEvents').get();
        const tentativeEvents = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
        res.json(tentativeEvents);
    } catch (err) {
        res.status(500).json({
            error: 'Internal server error',
        });
